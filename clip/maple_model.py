...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, masks=None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        ...
        if masks is not None:
            ...
            w = masks.flatten(1).to(x.device)
            x = x * (1 - w.unsqueeze(-1))     # Feed masks with sparse tokens
            # x = x[(1 - w).type(torch.bool)].view(x.shape[0], -1, x.shape[-1])
